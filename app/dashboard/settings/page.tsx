...
function SettingsContent() {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      const targetLeft = activeTrigger.offsetLeft - (scroller.clientWidth - activeTrigger.offsetWidth) / 2;
        ...
        <div ref={tabScrollerRef} className="-mx-1 overflow-x-auto px-1 pb-1">
          <TabsList className="h-auto w-max justify-start gap-1 rounded-lg border border-border/70 bg-card p-1 shadow-sm">
            <TabsTrigger
              ...
              className="h-9 gap-1.5 rounded-md px-3 text-xs font-medium whitespace-nowrap data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
            >
