...
export default function CustomerDetailPage({ customer }: { customer: Record<string, unknown> }) {
    ...
    const handleDeleteNoteConfirm = async () => {
        ...
        try {
            const response = await fetch(`https://api.gajkesaristeels.in/notes/delete?id=${notePendingDelete.id}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`,
                },
    ...
    const renderPaginationItems = (tab: keyof typeof currentPage) => {
        ...
        switch (tab) {
            case 'visits':
                dataLength = filteredVisitsData.length;
                break;
            case 'notes':
    ...
    useEffect(() => {
        setCurrentPage(prev => {
            const totalPages = Math.max(1, Math.ceil(filteredVisitsData.length / ITEMS_PER_PAGE));
            if (prev.visits > totalPages) {
                return { ...prev, visits: totalPages };
            }
            return prev;
        });
    }, [filteredVisitsData.length]);
                            ...
                            <div className="space-y-6">
                                ...
                                {activeActivityTab === 'visits' && (
                                    <div className="space-y-4">
                                        ...
                                        {showMore.visits && filteredVisitsData.length > ITEMS_PER_PAGE && (
                                            <Pagination>
                                                <PaginationPrevious
                                                    size="default"
                                                ...
                                                <PaginationNext
                                                    size="default"
                                                    onClick={currentPage.visits === Math.ceil(filteredVisitsData.length / ITEMS_PER_PAGE) ? undefined : () => setCurrentPage(prev => ({ ...prev, visits: Math.min(prev.visits + 1, Math.ceil(filteredVisitsData.length / ITEMS_PER_PAGE)) }))}
                                                />
                                            </Pagination>
                                        )}
                                        {filteredVisitsData.length > ITEMS_PER_PAGE && (
