        ...
        return (
          <article key={task.id} className="overflow-hidden rounded-lg border bg-card">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 border-t bg-muted/20 px-4 py-2.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5"><User className="h-3.5 w-3.5 shrink-0 text-primary" /><span className="font-medium text-foreground">{task.assignedTo || 'Unassigned'}</span></div>
              {due && <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 shrink-0" /><span>Due {due}</span></div>}
            </div>
          </article>
        );
      })}
